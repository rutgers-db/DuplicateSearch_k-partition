...
public:
    ...
    void queryLongest(int node, int L, int R, double val, int &res) {
        if (minv[node] >= val) {
            res = R;
            return;
        }
    ...
    void queryLongestWithLL(int node, int L, int R, int LL, double val, int &res) {
        if (L > R || L > LL || R < LL){
            // If the current segment is outside the query range [LL, end]
            return;
        }
        ...
        push_down(node);
        int mid = (L + R) >> 1;
        queryLongestWithLL(node << 1 | 1, mid + 1, R, LL, val, res);
        if (res < mid + 1)
            queryLongestWithLL(node << 1, L, mid, LL, val, res);
    }
