...
int main(int argc, char *argv[])
{
    ...
    if (dataset == "PAN")
    {   
        pair_file_path = "/path/to/your/data/sample_source_sus_pairs.txt";
        source_file_path = "/path/to/your/data/all_source_with_plagiarism.bin";
        query_file_path = "/path/to/your/data/all_suspicious_seqs_pan11.bin";
...
#pragma omp parallel for
    for (int i = 0; i < queryNum; i++)
    {
        ...
        KMINS kmins(q_seq, s_seq, k, threshold, hasher);
        kmins.init();
        auto time_st = timerStart();
        CWSNum[tid] += kmins.run(if_intervalscan, if_longest);
        ave_timeCost[tid] += timerCheck(time_st);
        results_num[tid] += kmins.result.size();
        // auto res = kmins.filter_covered_ranges();
