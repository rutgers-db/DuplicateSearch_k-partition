...
void nearDupSearch(vector<vector<int>> &docs, unordered_map<int, vector<CW>> &tidToCW, float threshold, unordered_map<int, vector<tuple<int, int>>> &results) {
    SegmentTree segtree;
...
int main(int argc, char *argv[]) {
    ...
    char out_file[100];
    sprintf(out_file, "ExpResults/EXP3/OPH_longest_filter_%s_n%d_l%d_k%d_t%.1f_q%d.txt", src_file.substr(src_file.rfind('/') + 1, src_file.rfind('.') - src_file.rfind('/') - 1).c_str(), doc_num, doc_length, k, threshold, queryNum);
